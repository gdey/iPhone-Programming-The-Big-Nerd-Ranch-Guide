...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
	   ...
	   // Create the new layer object 
		...
		// Create a CGColorRef that is red with transparency 
		CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    
		float redComponents[] = {1,0,0,0.5}; 
		CGColorRef redish = CGColorCreate(colorSpace, redComponents); 
		// Make that the background color for the layer 
		[boxLayer setBackgroundColor:redish]; 
		CFRelease(redish); 
...
- (void)didMoveToWindow {
	...
	if ([self window]) {
		...
		// Must be first responder to get motion events
